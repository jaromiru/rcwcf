...
#==============================
# changed to reveal a complete branch
class ItemMask:
	def __init__(self, item, meta, all_visible=False):
		self.orig_item = item
		...
		for f, m in enumerate(self.mask):
			...
			if m > 0:
				...
				self.init_cost += meta.costs[f]

			# process bags
			bid = meta.bag_idx[f]
			if bid is not None: # it is a BAG
				if m > 0 or (meta.costs[f] == 0 and all_visible):
					bag_items = [ItemMask(it, meta.bags[bid][1], all_visible) for it in item.bags[bid]]
				else:
					bag_items = []

				self.bags.append(bag_items)
				self.init_cost += sum([x.init_cost for x in self.bags[bid]])
		...
		if bid is None:
			...
			if not self.bags[bid]:	# expand
				...
				if self.orig_item.bags[bid]:
					self.bags[bid] = [ItemMask(it, meta.bags[bid][1], all_visible=True) for it in self.orig_item.bags[bid]]
				...
				r = sum([x.init_cost for x in self.bags[bid]])
				return meta.costs[f] + r
...
class RandomItemMask:
	def __init__(self, item, meta, rand_r):
		...
		for f, feat_idx in enumerate(meta.feat_idx):
			...
			bid = meta.bag_idx[f]
			if bid is not None: # it is a BAG
				all_visible = np.random.rand() < rand_r
				bag_items = [ItemMask(it, meta.bags[bid][1], all_visible) for it in item.bags[bid]]
				self.bags.append(bag_items)

...
class Net(torch.nn.Module):
	...
	def train(self, batch):
		res = self(batch)
		batch_y = torch.tensor([x.label for x in batch]).to(config.DEVICE)

		loss = self.loss(res, batch_y)

		# res_p = torch.log_softmax(res, 1)
		# loss = -res_p.gather(1, batch_y.view(-1, 1)).mean()

#!/bin/bash
#SBATCH -p cpulong --time=72:00:00
#SBATCH -n1 -c1
#
#SBATCH --array=0-29
...
DATE=`date +%d_%m_%Y`
WORK_DIR=${EXEC_DIR}/${EXP_NAME}/${EXP}_${DATE}_flat
mkdir -p $WORK_DIR

cp ${EXEC_DIR}/agentr_flat/config.py $WORK_DIR/
...
echo srun -o $WORK_FILE -D $WORK_DIR python -u ${EXEC_DIR}/agentr_flat/main.py $EXP $lambda -eplen $EPLEN -epochs $EPOCHS -batch $BATCH -log $LOG_FILE -model $MODEL_FILE -alpha_h $A_H -alpha_h_min $A_H_MIN -l2 $L2 
srun -o $WORK_FILE -D $WORK_DIR python -u ${EXEC_DIR}/agentr_flat/main.py $EXP $lambda -eplen $EPLEN -epochs $EPOCHS -batch $BATCH -log $LOG_FILE -model $MODEL_FILE -alpha_h $A_H -alpha_h_min $A_H_MIN -l2 $L2 

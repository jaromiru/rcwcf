...
class BagInput(torch.nn.Module):
	def __init__(self, meta, act_f, agg_f):
		...
		for fid, bag in meta.bags:
			module = BagInput(bag, F.relu, seg_mean_vec) # <- can be replace with seg_mean_vec
...
class Net(torch.nn.Module):
	...
	def get_loss(self, s, a, r, s_, y, net_res, flag):
		...
		loss_pi = -adv_dtc * log_pi
		loss_v  = torch.mul(adv, adv)
		...
		return loss_pi, loss_v, loss_h, loss_cls

	def train_(self, s, a, r, s_, y, net_res, flag, info):
		loss_pi, loss_v, loss_h, loss_cls = self.get_loss(s, a, r, s_, y, net_res, flag)
